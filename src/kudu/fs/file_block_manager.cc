...
class FileWritableBlock : public WritableBlock {
 public:
  ...
  virtual Status fclone(const BlockId& block_id,ArrayView<const Slice> data) OVERRIDE;
...
Status FileWritableBlock::fclone(const BlockId& block_id,ArrayView<const Slice> data){
  string path;
  
  /*if (!block_manager_->FindBlockPath(block_id, &path)) {
